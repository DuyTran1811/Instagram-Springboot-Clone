...
public class UserService implements IUserService {
    ...
    public void upDateUserName(Long id, User user) {
        ...
        oldUser.setStreetName(user.getStreetName());
        oldUser.setBuildingNumber(user.getBuildingNumber());
    ...
    public void registerUser(SignUpRequest signUpRequest) {
        ...
        if (strRoles == null) {
            Role roleUser = roleRepository.findRoleByName(ERole.USER).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
            roles.add(roleUser);
        } else {
            strRoles.forEach(role -> {
                switch (role) {
                    case "ADMIN":
                        Role adminRole = roleRepository.findRoleByName(ERole.ADMIN).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(adminRole);
                        break;
                    case "SERVICE":
                        Role serviceRole = roleRepository.findRoleByName(ERole.SERVICE).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(serviceRole);
                        break;
                    case "USER":
                        Role userRole = roleRepository.findRoleByName(ERole.USER).orElseThrow(() -> new RuntimeException("Error: Role is not found."));
                        roles.add(userRole);
                        break;
                }
            });
